...
- Clone/Fork the Repo. 
    ```
    git clone https://github.com/Priyanshudesk/argocd.git
...
Before creating pipeline, test locally if your dockerfile is accurate by running container.We will create Dockerfile.
...
```
docker build -t Priyanshudesk/demo-app:v1 .
```
...
2. Create your first pipeline for TEST and BUILD the image. make sure it should be yaml file
    ...
    build:
        ...
        - name: Build Docker Image
            run: |
            docker build . -t Priyanshudesk/demo-app:${{ env.VERSION }} 
        
        # Push the Docker Image
        - name: Push Docker Image
            run: |
            docker push Priyanshudesk/demo-app:${{ env.VERSION }}
        
        # UPdate the K8s Manifest Files
        - name: Update K8s Manifests
            run: |
            cat deploy/deploy.yaml
            sed -i "s|image: Priyanshudesk/demo-app:.*|image: Priyanshudesk/demo-app:${{ env.VERSION }}|g" deploy/deploy.yaml
            cat deploy/deploy.yaml
        ...
        - name: Commit the changes
            ...
            git remote set-url origin https://github-actions:${{ secrets.GITHUB_TOKEN }}@github.com/Priyanshudesk/Flask-App-GitHub-Actions-ArgoCD.git
